...
population <- DBI::dbGetQuery(connection, "
  ...
  select l.gameid, g.target_valid
...
market <- DBI::dbGetQuery(connection, "
  with verified_links as (
    ...
    select l.gameid, l.league_canonical, l.market_close_time,
           g.game_datetime, g.series_id,
...
raw_over <- 1 / as.numeric(market$odds_over)
raw_under <- 1 / as.numeric(market$odds_under)
market$market_probability_over <- raw_over / (raw_over + raw_under)
...
pair_features <- function(blue, red) {
  ...
  fight_ceiling_values <- c(
    blue[["win_kpm"]] + red[["loss_dpm"]],
    red[["win_kpm"]] + blue[["loss_dpm"]]
  )
  fight_ceiling <- if (all(!is.finite(fight_ceiling_values))) {
    NA_real_
  } else {
    max(fight_ceiling_values, na.rm = TRUE)
  }
  c(
    ...
    cross_fight_ceiling = clean(fight_ceiling),
    minimum_history = clean(min(
      blue[["history_games"]],
      red[["history_games"]],
      na.rm = TRUE
    )),
    minimum_win_history = clean(min(
      blue[["win_games"]],
      red[["win_games"]],
      na.rm = TRUE
    )),
    minimum_loss_history = clean(min(
      blue[["loss_games"]],
      red[["loss_games"]],
      na.rm = TRUE
    ))
...
required <- c(
  "observed_total", "line", "market_mean", "blue_team_id", "red_team_id"
)
data <- data[stats::complete.cases(data[, required, drop = FALSE]), , drop = FALSE]
coverage <- rbind(
  coverage,
  data.frame(
    stage = c("team_identity_joined", "excluded_after_team_join"),
...
for (month_value in months) {
  ...
  prediction_rows[[as.character(month)]] <- data.frame(
    ...
    intercept_mean = pmax(
      validation$market_mean + mean(train$residual_target),
      0.1
    ),
...
intercept_scores <- score_distribution(
  predictions$observed_total,
  predictions$line,
  predictions$intercept_mean,
  market_theta
)
for (metric in names(market_scores)) {
  predictions[[paste0("market_", metric)]] <- market_scores[[metric]]
  predictions[[paste0("intercept_", metric)]] <- intercept_scores[[metric]]
...
for (metric in c("crps", "count_log_score", "absolute_error", "brier")) {
  ...
  predictions[[paste0("delta_vs_intercept_", metric)]] <-
    predictions[[paste0("candidate_", metric)]] -
    predictions[[paste0("intercept_", metric)]]
}
...
for (league in sort(unique(predictions$league_canonical))) {
  rows <- predictions[predictions$league_canonical == league, , drop = FALSE]
...
paired_bootstrap <- function(
  ...
  result <- lapply(metrics, function(metric) {
    difference_field <- if (comparison == "market") {
      paste0("delta_", metric)
    } else {
      paste0("delta_vs_intercept_", metric)
    }
    values <- rows[[difference_field]]
...
bootstrap_rows <- list(
  overall = transform(
    paired_bootstrap(predictions, comparison = "market"),
    sample_scope = "all_oof_maps"
  ),
  overall_intercept = transform(
    paired_bootstrap(predictions, comparison = "intercept"),
    sample_scope = "all_oof_maps"
  )
)
for (scope in unique(predictions$sample_scope)) {
  ...
  bootstrap_rows[[scope]] <- transform(
    paired_bootstrap(
      rows,
      comparison = "market",
      seed = 20260805L + nchar(scope)
    ),
    sample_scope = scope
  )
  bootstrap_rows[[paste0(scope, "_intercept")]] <- transform(
    paired_bootstrap(
      rows,
      comparison = "intercept",
      seed = 20260825L + nchar(scope)
    ),
    sample_scope = scope
  )
}
...
report <- c(
  ...
  "",
  paste(
    "A familia win_loss_behavior e lambda 1 foram congeladas antes deste",
    "teste. Os coeficientes foram reestimados mensalmente apenas com mapas",
    "anteriores ao mes avaliado."
  ),
  "",
  "## Universo e integridade",
  "",
  paste(
    "O universo bruto possui",
    coverage$maps[coverage$stage == "verified_target_valid"],
    "mapas verificados e validos. O teste produziu",
    nrow(predictions),
  ...
  paste(
    "No conjunto completo, o intercepto sozinho obteve CRPS",
    sprintf("%.6f", summary$intercept_crps[summary$group_type == "overall"]),
    "contra",
    sprintf("%.6f", summary$candidate_crps[summary$group_type == "overall"]),
    "da familia win/loss."
  ),
  "",
  "## Decisao",
